...
namespace Gisoft.NetTopologySuite.IO
{
    ...
    public class DbaseFileHeader
    {
        ...
        public void AddColumn(string fieldName, char fieldType, int fieldLength, int decimalCount)
        {
            ...
            switch (fieldType)
            {
                ...
                case 'i':
                    ...
                    if (fieldLength > 8) Trace.WriteLine("Field Length for " + fieldName + " set to " + fieldLength + " Preserving length, but should be set to Max of 8 for dbase III specification.");
                    tempFieldDescriptors[_fieldDescriptions.Length].Length = 8;
                    break;
                default:
        ...
        internal static Encoding GetEncoding(IStreamProvider provider = null)
        {
            ...
            string cpgText;
            using (var sr = new StreamReader(provider.OpenRead(),Encoding.Default))
