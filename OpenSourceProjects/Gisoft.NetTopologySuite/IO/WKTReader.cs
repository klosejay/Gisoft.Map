...
namespace Gisoft.NetTopologySuite.IO
{
    ...
    public class WKTReader : ITextGeometryReader
    {
        ...
        /// </summary>
        public WKTReader() : this(Gisoft.GeoAPI.GeometryServiceProvider.Instance.CreateGeometryFactory()) { }
        ...
        public static WKTReader Default { get; } = new WKTReader();
        /// <summary>
        /// Gets or sets the factory to create geometries
        /// </summary>
        public IGeometryFactory Factory
        {
            get { return Gisoft.GeoAPI.GeometryServiceProvider.Instance.CreateGeometryFactory(_precisionModel, DefaultSRID, _coordinateSequencefactory); }
            set
            {
                if (value != null)
                {
        ...
        public IGeometry Read(TextReader reader)
        {
            ...
            {
                throw new Gisoft.GeoAPI.IO.ParseException(e.ToString());
            }            
        }
        ...
        /// <exception cref="Gisoft.GeoAPI.IO.ParseException">if the next token is not a valid number</exception>
		private static double GetNextNumber(IEnumerator<Token> tokens)
        {
            ...
            if (token is EofToken)
                throw new Gisoft.GeoAPI.IO.ParseException("Expected number but encountered end of stream");
            if (token is EolToken)
                throw new Gisoft.GeoAPI.IO.ParseException("Expected number but encountered end of line");
            ...
            if (token is WordToken)
            {
                ...
                throw new Gisoft.GeoAPI.IO.ParseException("Expected number but encountered word: " + token.StringValue);
            }
            if (token.StringValue == "(")
                throw new Gisoft.GeoAPI.IO.ParseException("Expected number but encountered '('");
            if (token.StringValue == ")")
                throw new Gisoft.GeoAPI.IO.ParseException("Expected number but encountered ')'");
            if (token.StringValue == ",")
                throw new Gisoft.GeoAPI.IO.ParseException("Expected number but encountered ','");

            throw new Gisoft.GeoAPI.IO.ParseException("Expected number but encountered '" + token.StringValue + "'");
        }
        ...
        private static string GetNextEmptyOrOpener(IEnumerator<Token> tokens) 
        {
            ...
            if (nextWord.Equals("EMPTY") || nextWord.Equals("(")) 
                return nextWord;
            throw new Gisoft.GeoAPI.IO.ParseException("Expected 'EMPTY' or '(' but encountered '" + nextWord + "'");
        }
        ...
        private static string GetNextCloserOrComma(IEnumerator<Token> tokens) 
        {
            ...
            throw new Gisoft.GeoAPI.IO.ParseException("Expected ')' or ',' but encountered '" + nextWord
                + "'");
        }
        ...
        private static string GetNextCloser(IEnumerator<Token> tokens) 
        {
            ...
            if (nextWord.Equals(")"))
                return nextWord;
            throw new Gisoft.GeoAPI.IO.ParseException("Expected ')' but encountered '" + nextWord + "'");         
        }
        ...
        private static string GetNextWord(IEnumerator<Token> tokens)
        {
            ...
            if (token is EolToken)
                throw new Gisoft.GeoAPI.IO.ParseException("Expected number but encountered end of line");
            if (token is FloatToken || token is IntToken)
                throw new Gisoft.GeoAPI.IO.ParseException("Expected word but encountered number: " + token.StringValue);
            if (token is WordToken)
        ...
        internal IGeometry ReadGeometryTaggedText(IEnumerator<Token> tokens)
        {
            ...
            var factory = Gisoft.GeoAPI.GeometryServiceProvider.Instance.CreateGeometryFactory(_precisionModel, srid,
            ...
            else throw new Gisoft.GeoAPI.IO.ParseException("Unknown type: " + type);
